...
class PlanetMap:
    ...
    def setup(self, coll_pos):
        self.planet_player = PlanetPlayer((50, 350), self.all_sprites, coll_pos) # теперь в аргументах указываем ещё и позиции осязаемых объектов
    ...
    def draw(self, dt):
        ...
        if self.stat == 0: # меняем необходимые флаги
            self.setup(self.coll) # передаём позиции осязаемых позиций
            self.stat = 1
            self.stat2 = 0
